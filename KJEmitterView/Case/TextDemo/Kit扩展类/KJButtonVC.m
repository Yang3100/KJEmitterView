...
- (void)viewDidLoad {
    ...
    self.segmentedTitleArray = @[
        @[@"居中-图左文右",@"居中-图右文左",@"居中-图上文下",@"居中-图下文上"],
...
- (UIButton *)button{
    if (!_button) {
        ...
        UIImage *image = [[UIImage imageNamed:@"wode_nor"] kj_imageLinellaeColor:UIColor.greenColor];
...
- (UIButton *)emitterButton{
    if (!_emitterButton) {
        ...
        [_emitterButton setImage:[UIImage imageNamed:@"button_like_norm"] forState:UIControlStateNormal];
        [_emitterButton setImage:[UIImage imageNamed:@"button_like_sele"] forState:UIControlStateSelected];
...
- (UIButton *)countDownButton{
    if (!_countDownButton) {
        ...
        [_countDownButton kj_countDownTimeStop:^{
            NSLog(@"计时结束!!!");
        }];
    }
