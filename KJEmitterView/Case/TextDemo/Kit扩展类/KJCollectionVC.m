...
- (void)viewDidLoad {
    ...
    UILabel *label = [UILabel createLabel:^(id<KJLabelDelegate>  _Nonnull handle) {
        handle.chainAddView(self.view);
        handle.chainText(@"核心就是解决左右滚动和上下滑动冲突处理").chainFont([UIFont systemFontOfSize:16]).chainTextColor(UIColor.orangeColor);
    }];
    [label sizeToFit];
...
- (UICollectionView*)collectView{
    if (_collectView == nil) {
        ...
        [_collectView moveWithBlock:^(KJMoveStateType state, CGPoint point) {
